...
const TrendingProducts = () => {
    ...
    const trendingProducts = products.filter(item => item.status === 'accepted' && item.featured == true);
    ...
    return (
        <div className="my-10">
            ...
            {
                trendingProducts.map(item => <div key={item._id} className="card bg-base-100 border">
                ...
                <div className="card-body">
                  ...
                  <div>Tags: {item.tags.map(tag => <div key={tag.index} className="badge badge-secondary badge-outline mr-2">{tag}</div>)}</div>
        ...
        <div className="my-5 flex justify-center">
            <Link to='/products'><button className="btn btn-primary"> Show All Products</button></Link>
        </div>
        </div>
    );
};
